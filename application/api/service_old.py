    ...
    if question.intent_ner_recognition:
        ...
        if entity_slot:
            for each_entity in entity_slot:
                entity_retrieve = get_retrieve_opensearch(each_entity, "ner", question.profile_name, 1,
                                                          0.7)
                if entity_retrieve:
    ...
    if search_intent_flag:
        ...
        if search_intent_result["status_code"] == 500:
            ...
            if search_intent_result["data"] is not None and len(search_intent_result["data"]) > 0:
                search_intent_result["data"] = search_intent_result["data"]
...
async def ask_websocket(websocket: WebSocket, question: Question):
    ...
    if ask_replay_flag:
        ...
        answer = Answer(query=search_box, query_rewrite=ask_result.query_rewrite, query_intent="ask_in_reply",
                        knowledge_search_result=knowledge_search_result,
                        sql_search_result=sql_search_result, agent_search_result=agent_search_response,
    ...
    if reject_intent_flag:
        ...
        LogManagement.add_log_to_database(log_id=log_id, user_id=user_id, session_id=session_id,
                                          profile_name=selected_profile, sql="", query=search_box,
                                          intent="reject_search", log_info=reject_answer_info, time_str=current_time, log_type="chat_history")
        return answer
    elif search_intent_flag:
        ...
        agent_cot_retrieve = get_retrieve_opensearch(query_rewrite, "agent",
                                                     selected_profile, 2, 0.5)
    ...
    if search_intent_flag:
        ...
        if search_intent_result["status_code"] == 500:
            ...
            normal_search_result = await normal_sql_regenerating_websocket(websocket=websocket, session_id=session_id,
                                                                           search_box=query_rewrite,
        ...
        if search_intent_result["status_code"] == 500:
            ...
            if search_intent_result["data"] is not None and len(search_intent_result["data"]) > 0:
                ...
                await response_websocket(websocket, session_id, "Data Visualization", ContentEnum.STATE, "end",
                                         user_id)
